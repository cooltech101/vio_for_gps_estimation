...
START_LAT = 1.3816667
START_LON = 103.888667
START_YAW = 5*2*math.pi/12
...
def init_csv_logger(csv_path: str):
    """
    Open CSV file and write header if the file is new or empty.
    """
...
def log_latlonalt_to_csv(
    ...
    """
    Log new latitude, longitude, and altitude to CSV.
    """
    csv_writer.writerow([
        f"{timestamp:.6f}",
        f"{new_lat:.10f}",
...
def translate_relative_xyz_to_latlonalt_geodesic(
    ...
    if distance_m == 0:
        return reference_lat, reference_lon, reference_alt
...
def main():
    ...
    parser.add_argument(
        "--csv",
        default=DEFAULT_CSV,
        help=f"CSV file path to log new lat/lon/alt. Default: {DEFAULT_CSV}",
    )
    ...
    print(f"[listener] Logging CSV to {csv_path}")
    ...
    try:
        while running:
            ...
            new_lat, new_lon, new_alt = translate_relative_xyz_to_latlonalt_geodesic(
                reference_lat=START_LAT,
                reference_lon=START_LON,
                reference_alt=1,
                vio_x=py,
                vio_y=px,
                vio_z=pz,
                reference_yaw_rad=START_YAW
            )

            log_latlonalt_to_csv(
